...
test('findOutOfStock lists inventory with zero quantity', {
  skip: !dependenciesAvailable,
}, async (t) => {
  const database = require('../src/config/database');
  const inventoryService = require('../src/services/inventoryService');
  ...
  const inventory = await inventoryService.findOutOfStock({ branchId: '3' });

  assert.match(selectSql, /inventory\.quantity = 0/);
  assert.match(selectSql, /inventory\.branch_id = \$1/);
  assert.deepEqual(selectParams, [3]);
  assert.equal(inventory[0].quantity, 0);
  assert.equal(inventory[0].lowStockAlert, true);
  assert.equal(inventory[0].product.name, 'Coffee');
});
