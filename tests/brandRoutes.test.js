...
test('brand routes register CRUD endpoints', {
  skip: !dependenciesAvailable,
}, () => {
  const brandRoutes = require('../src/routes/brandRoutes');

  const hasListRoute = brandRoutes.stack.some(
    (layer) => layer.route?.path === '/' && layer.route.methods.get === true
  );
  const hasCreateRoute = brandRoutes.stack.some(
    (layer) => layer.route?.path === '/' && layer.route.methods.post === true
  );
  const hasUpdateRoute = brandRoutes.stack.some(
    (layer) => layer.route?.path === '/:id' && layer.route.methods.put === true
  );
  const hasDeleteRoute = brandRoutes.stack.some(
